"""
Supermicro adapter selection 회귀.
...
| 3 | Supermicro / X12 / firmware=01.03.03                        | redfish_supermicro_x12       | priority 90 → 100 fix |
...
| 9 | Supermicro / model='' / firmware='' (facts empty 시나리오)  | redfish_supermicro_x14       | 현재 동작 — facts empty 시 priority 최상위 |
| 10 | Supermicro / model=Unknown / firmware='' (generic fallback) | redfish_supermicro_bmc       | priority 10 fallback |
| 11 | Dell / model='' / firmware='' (타 vendor 영향 0)            | redfish_dell_idrac10         | 회귀 영향 0 |
| 12 | HPE / model=Gen11 / firmware=1.73 (HPE 핵심 회귀)           | redfish_hpe_ilo6             | fix 무영향 |

조건 #9 (현재 동작) 는 lab 도입 후 firmware_patterns 추가 시 정밀 분리 가능.
조건 #11 / #12 는 본 fix 가 HPE + Dell/Lenovo/Cisco 회귀에 영향 0 임을 확인.
"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # X12 generation (AST2600) — fix 핵심 (priority 90 → 100)
        ...
        # facts empty 시나리오 (model + firmware empty) — 현재 동작 검증
        ...
        # HPE 회귀 (probe_facts 시뮬레이션 — 사이트 실측 hint 채워진 케이스)
...
def test_supermicro_adapter_selection(case_id: str, facts: dict, expected_adapter: str) -> None:
    """Supermicro X11~X14 adapter 선택 회귀 + priority fix 검증 + 타 vendor 영향 0."""
...
def test_drift_015_x12_priority_consistency() -> None:
    """X11/X12/X13 priority 모두 100 일관성. X14=110 > X11~X13=100 > generic=10."""
    ...
    assert adapters["redfish_supermicro_x12"]["priority"] == 100, "X12 priority 100 (이전 90 회귀 차단)"
