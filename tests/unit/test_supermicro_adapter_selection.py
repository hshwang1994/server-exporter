...
| 3 | Supermicro / X12 / firmware=01.03.03                        | redfish_supermicro_x12       | priority 100 |
...
| 11 | Dell / model='' / firmware='' (타 vendor 미간섭)            | redfish_dell_idrac10         | Dell 선택 유지 |
| 12 | HPE / model=Gen11 / firmware=1.73 (HPE 핵심 회귀)           | redfish_hpe_ilo6             | HPE 선택 유지 |

조건 #9 (현재 동작) 는 lab 도입 후 firmware_patterns 추가 시 정밀 분리 가능.
조건 #11 / #12 는 Supermicro 선택 로직이 HPE + Dell/Lenovo/Cisco 선택에 간섭하지 않음을 확인.
"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # X12 generation (AST2600) — priority 100
...
def test_supermicro_adapter_selection(case_id: str, facts: dict, expected_adapter: str) -> None:
    """Supermicro X11~X14 adapter 선택 회귀 + priority 검증 + 타 vendor 미간섭."""
