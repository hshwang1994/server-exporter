...
> 이 폴더는 서버별 "정답지" 입니다. server-exporter 가 정상 동작했을 때의 출력 JSON 을 그대로 보존했습니다.
...
- 식별자는 serial_number, system_uuid 같은 DMI 정보입니다. 권장 운영 모드 (sudo become 사용) 기준으로 기록되어 있습니다.
...
라인별 한글 주석본은 본 폴더에 두지 않는다. **`schema/output_examples/`** 에 채널/벤더 대표 예시로 보존된다 (아래 표 "한글 주석 예시" 열). JSON 표준은 주석을 허용하지 않아 확장자가 `.jsonc` 다.
...
2. **baseline JSON 의 `diagnosis.details.baseline_origin`** 필드 — 출처 + cycle + NEXT_ACTIONS 교체 의무를 명시
...
- NEXT_ACTIONS (내부 후속 작업 목록) 의 C1~C8 등 사이트 fixture 캡처 후속 작업이 진행되면 mock-derived baseline 은 실측으로 교체 의무 (rule 13 R4 정신)
- mock-derived 가 실측 baseline 으로 잘못 인용되는 사고를 막으려고 `diagnosis.details.baseline_origin` 자동 검사 hook 을 넣을지 검토한다 (NEXT_ACTIONS, 미래 작업)
...
본 폴더의 파일은 절대 in-place 로 덮어쓰지 않는다. 그래야만 회귀의 의미가 보존됩니다.
