...
| 파일 | 대상 | 특이사항 |
|---|---|---|
| `os_linux_rhel810_raw_fallback.jsonc` | RHEL 8.10 (10.100.64.161) | **Linux 2-tier raw fallback** — Python 3.6.8 → setup 모듈 미동작 → `raw` 모듈로 대체 |
...
| `redfish_hpe_ilo6.jsonc` | HPE ProLiant DL380 Gen11 / iLO 6 v1.73 (10.50.11.231) | 캡처에는 `redfish_hpe_ilo7` 로 찍혀 있다 (fix 이전 캡처). 현재 코드는 `redfish_hpe_ilo6` 를 선택한다 — 재캡처 필요 |
| **`redfish_hpe_csus_3200.jsonc`** | **HPE Compute Scale-up Server 3200 — Mock 합성 (lab 부재 / sdflexutils + DMTF v1.15 + iLO5 API ref 합성)** | `redfish_hpe_csus_3200` (priority=102 / RMC primary / data.multi_node 활성) |
...
- 펌웨어 / 환경 변경 시 본 디렉터리 직접 갱신
- Round 검증 후 baseline 과 함께 갱신
- 6개월 동안 갱신 0건 시 stale 가능 — 외부 시스템 계약과 동기화 권장

## 7. 출처

- 캡처 일시: 2026-05-07
- 캡처 환경: Jenkins 에이전트 10.100.64.155 (Ansible 2.20.3 / Python 3.12 venv `/opt/ansible-env/`)
- 자격증명: `vault/{linux,windows,esxi}.yml` + `vault/redfish/{vendor}.yml` (5 vendor 통일 — primary `infraops/Passw0rd1!Infra` + recovery)

## 8. 호출자 사용 가이드

호출자 시스템 (Jenkins downstream / 모니터링 / 포털) 은:
1. envelope 13 필드 fixed — shape 변경 0
2. `status` 으로 1차 분기 (success / partial / failed)
3. `sections.<섹션>` 으로 2차 분기 (success / failed / not_supported)
4. `errors[]` 가 있어도 `status=success` 일 수 있음 (warning 수준 — 시나리오 B)
