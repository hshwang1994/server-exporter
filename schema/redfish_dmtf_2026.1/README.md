...
> **런타임 코드는 이 파일들을 import 하지 않는다.** 의존성이 아니라 순수 reference 데이터다.
...
- **포맷**: JSON Schema. enum/필드 대조에 json-schema 가 충분해서 CSDL XML / OpenAPI 는 미포함.

## 범위 (전체 번들 아님)

전체 번들은 약 14,000 파일(json-schema 6,890 + CSDL 279 + OpenAPI 6,863 + registries)이고,
본 디렉터리는 server-exporter 가 실제 참조하는 리소스만 subset 으로 보관한다.

각 리소스마다 2개 파일:
- `<Resource>.json` — 무버전(consolidated). 일부 리소스는 enum 을 인라인 포함(예: Volume).
- `<Resource>.v1_X_Y.json` — 최신 버전. enum definition 이 인라인으로 들어있어 대조의 정본.
...
> `Thermal` / `ThermalSubsystem` = 현재 코드 미구현, fallback 후보라 참조용 포함.
...
갱신 시: 동일 절차로 해당 리소스 최신 json-schema 를 재복사하고 `dmtf_info.json` 버전을
갱신한 뒤, 내부 외부계약 카탈로그 의 대조 스냅샷을 갱신한다.

## 최초 대조 결과 (2026-06-08)

`redfish_gather.py` 의 enum/필드/path 10개 대조 항목 중 9 MATCH + 1 GAP 이고, GAP 은 보정을
마쳤다. 상세는 내부 외부계약 카탈로그 "DMTF DSP8010 2026.1 대조" 절 참조.
