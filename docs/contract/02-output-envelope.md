...
`reachable`은 **관리 TCP 응답 또는 ICMP Echo 응답**이다 (2026-09-03). 순서가 있다 —
관리 포트를 먼저 보고, TCP가 아무 응답도 주지 않았을 때만 Echo를 한 번 더 보낸다.
핑은 게이트가 아니라서, 관리망이 핑을 막아 둬도 BMC가 443으로 답하면 그대로 통과한다.
반대로 방화벽이 관리 포트 TCP를 조용히 버리는 구간에서는 Echo 응답이 "장비는 있다"를
증명해 실패가 `reachable`이 아니라 `port` 단계로 내려간다.
