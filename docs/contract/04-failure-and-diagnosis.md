...
따로 처리할 필요가 없고, 대신 봉투 안의 몇 개 필드로 무슨 일이 있었는지 읽으면 된다.
...
`DNS_RESOLUTION_FAILED`가 목록에 있지만, 이 시스템은 IPv4만 받는다. 그래서 사용자
안내에서 DNS나 호스트명 확인을 권하지 않는다.
...
길이는 2000자에서 잘린다. 로그 전체가 들어오지는 않는다는 뜻이니, 원인 추적은 Jenkins
콘솔 로그를 함께 봐야 한다.
