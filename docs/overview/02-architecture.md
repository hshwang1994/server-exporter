...
여기서 중요한 게 하나 있다. **핑은 게이트가 아니다.** 관리망은 핑을 막아 두는 경우가
흔한데, 핑을 앞단 관문으로 쓰면 멀쩡히 443으로 답하는 BMC를 죽은 장비로 오판한다.
그래서 관리 포트를 먼저 보고, TCP가 아무 응답도 주지 않았을 때만 ICMP Echo를 한 번 더
보낸다 — `reachable`은 **TCP 응답 OR ICMP 응답**이다 (2026-09-03).

이 순서가 양쪽 오판을 다 막는다. 핑이 막혀 있어도 TCP가 답하면 통과이고, 방화벽이 관리
포트 TCP를 조용히 버려도 Echo가 오면 "장비는 있는데 관리 포트를 못 연다"로 구분된다.
후자는 `reachable` 실패가 아니라 `port` 단계 실패이고, 운영자에게 나가는 문장도
"대상 서버와 통신은 되지만 관리 포트에 연결할 수 없습니다. 방화벽과 접속 설정을 확인하세요."다.
...
출력 플러그인이 태스크 이름을 문자열 그대로 비교하기 때문에 `OUTPUT: 결과` 같은
변형은 잡히지 않는다 (`callback_plugins/json_only.py:366`). 그 플러그인이 Ansible의
다른 출력을 전부 막고 이 JSON만 표준출력으로 내보낸다.
